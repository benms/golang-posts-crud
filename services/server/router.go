...
func setRouter(cfg conf.Config) *gin.Engine {
	...
	router.GET("/", func(ctx *gin.Context) { ctx.JSON(http.StatusOK, gin.H{"msg": "Pharos server"}) })
	...
	router.NoRoute(func(ctx *gin.Context) { ctx.JSON(http.StatusNotFound, gin.H{"msg": "Route not found"}) })
	return router
}
